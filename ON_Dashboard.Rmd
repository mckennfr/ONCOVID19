...
daily_confirmed$p30 <- 100*exp(.3*daily_confirmed$date)
daily_confirmed$p20 <- 100*exp(.2*daily_confirmed$date)
daily_confirmed$p10 <- 100*exp(.1*daily_confirmed$date)

daily_confirmed[daily_confirmed > 2000000] <- NA
...
daily_confirmed %>%
  ...
  plotly::add_trace(
    ...
    type = "scatter",
    mode = "lines",
    line = list(color = 'rgb(80,80,80)', width = 2, dash = 'dot'),
    hoverinfo = FALSE,
    name = "30%"
  ) %>%
  plotly::add_trace(
...
mod1 <- nls(log(Y) ~ Const + r * X + I(log(K - Y)), 
            data = df3[-(1:8),], 
            start = c(Const = 1, r = 0.25, K = 1.2*max(df3$Y)+0.1))
